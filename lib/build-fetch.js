...
async function buildFetch ({ schema, method, id, body, verb, opts = {} } = {}) {
  const { fastGlob, error, currentLoc, importModule } = this.bajo.helper
  const { merge, cloneDeep, get, each } = this.bajo.helper._
  ...
  url = url.replace('{collName}', schema.collName)
